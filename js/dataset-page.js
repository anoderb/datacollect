...
async function pipelineProcessAndQueue(imageBlob, fileName = null) {
  try {
...
async function captureSinglePhoto() {
  ...
  try {
    ...
    await pipelineProcessAndQueue(rawBlob);
...
async function handleUploadSelectedPhotos() {
  ...
  for (let i = 0; i < total; i++) {
    ...
    await pipelineProcessAndQueue(file, file.name);
...
function renderUploadProgress(stats) {
  ...
  if (stats.uploadingCount > 0 || stats.pendingCount > 0) {
    datasetSubtitleDisplay.innerHTML = `
      ...
      Mengunggah data: ${stats.completedCount} berhasil, ${stats.pendingCount} antre (${stats.progressPercent}%)
    `;
    ...
    // Kembalikan ke format info normal jika antrean kosong
...
function renderGallery() {
  ...
  datasetPhotos.forEach((photo) => {
    ...
    card.innerHTML = `
      <img src="${data.publicUrl}" alt="${photo.file_name}" loading="lazy" onerror="this.src='${photo.publicUrl || ''}'">
    ...
    // Aksi hapus foto dengan modal custom confirm
